...
import { getNearestRecordForUser } from "../../../domain/records.service.js";
...
export function registerCommands(bot) {
    ...
    bot.command("record", async (ctx) => {
        ...
        const nearest = await getNearestRecordForUser({
            companyId,
        ...
        const next = nearest.record;

        const isConfirmed = Number(next?.attendance) === 1 || Number(next?.attendance) === 2;
        ...
        if (!isConfirmed) {
            ...
            reply_markup = new InlineKeyboard()
                .text("✅ Подтвердить запись", `rec_confirm:${companyId}:${next.id}`)
                .text("❌ Отменить", `rec_cancel:${companyId}:${next.id}`);
        }
