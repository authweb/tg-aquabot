...
async function safeEditMessage(ctx, text) {
  try {
    await ctx.editMessageText(text, {
      ...
      reply_markup: { inline_keyboard: [] }, // убираем кнопки в том же сообщении
    });
    return true;
  } catch (e) {
...
// На случай, если editMessageText не сработает (например, сообщение "не изменилось"),
// можно отдельно попытаться убрать клавиатуру.
async function safeClearMarkup(ctx) {
  try {
...
export function registerCallbacks(bot) {
  bot.on("callback_query:data", async (ctx) => {
    ...
    // быстро закрыть "часики" Telegram
    ...
    try {
      ...
      if (!result?.ok) {
        ...
        // попытка заменить текст + убрать кнопки
        const edited = await safeEditMessage(ctx, msg);
        if (!edited) await safeClearMarkup(ctx);
        return;
      }
      ...
      const edited = await safeEditMessage(ctx, text);
      if (!edited) await safeClearMarkup(ctx);
      ...
      const edited = await safeEditMessage(ctx, BOT_TEXT.cbTimeout);
      if (!edited) await safeClearMarkup(ctx);
    }
  });
}